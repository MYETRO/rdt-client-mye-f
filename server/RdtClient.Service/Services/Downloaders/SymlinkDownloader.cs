...
namespace RdtClient.Service.Services.Downloaders
{
    public class SymlinkDownloader : IDownloader
    {
        ...
        public async Task<string?> Download()
        {
            ...
            // Combine the folder name with the target directory to create the combined path
            ...
            if (unWantedExtensions.Any(unwanted => "." + fileExtension == unwanted))
            {
                ...
                {
                    Error = $"Can't handle compressed files with symlink downloader"
                });
                return null;
            }
            ...
            if (file == null)
            {
                // If the file is not found in the combinedPath, attempt to find it in the original way
                var tries = 0;
